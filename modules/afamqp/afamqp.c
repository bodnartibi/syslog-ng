...
afamqp_worker_publish(AMQPDestDriver *self, LogMessage *msg)
{
  ...
  log_template_format(self->routing_key_template, msg, &self->template_options, NULL, LTZ_LOCAL,
                      self->super.seq_num,
                      NULL, routing_key);

  if (self->body_template)
    {
      log_template_format(self->body_template, msg, &self->template_options, NULL, LTZ_LOCAL,
                          self->super.seq_num,
                          NULL, body);
