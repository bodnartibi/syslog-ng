          ...
          for (gint i = 0; i < state->super.argc; i++)
            {
              ...
              log_template_format(state->super.argv[i], msg, args->opts, NULL, args->tz, args->seq_num, args->context_id, buf);
      ...
      for (gint i = 0; i < state->argc; i++)
        {
          ...
          /* NOTE: not recursive, as the message context is just one message */
          log_template_format(state->argv[i], msg, args->opts, NULL, args->tz, args->seq_num, args->context_id, buf);
          str_repr_encode_append(result, buf->str, buf->len, ",");
