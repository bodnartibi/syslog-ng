          ...
          for (i = 0; i < state->super.argc; i++)
            {
              ...
              log_template_append_format(state->super.argv[i], msg, args->opts, args->processed, args->tz, args->seq_num,
                                         args->context_id, result);
              first = FALSE;
            }
...
tf_if_call(LogTemplateFunction *self, gpointer s, const LogTemplateInvokeArgs *args, GString *result)
{
  ...
  if (filter_expr_eval_with_context(state->filter, args->messages, args->num_messages))
    {
      log_template_append_format_with_context(state->super.argv[0], args->messages, args->num_messages, args->opts,
                                              args->processed, args->tz,
                                              args->seq_num, args->context_id, result);
    }
  else
    {
      log_template_append_format_with_context(state->super.argv[1], args->messages, args->num_messages, args->opts,
                                              args->processed, args->tz,
                                              args->seq_num, args->context_id, result);
    }
}
